import tqdm
from openprompt.data_utils.text_classification_dataset import  TwtProcessor, AgnewsTitleProcessor, NewstitleProcessor, SnippetsProcessor, OhsumedProcessor
import torch
...
def write_time(filename, timestamp):
...
parser.add_argument("--model_name_or_path", default='/home/mjy/wangye/PLST/model')
...
if args.dataset == "twitter":
    dataset['train'] = TwtProcessor().get_train_examples("datasets/TextClassification/twitter/")
    dataset['test'] = TwtProcessor().get_test_examples("datasets/TextClassification/twitter/")
    ...
    dataset['train'] = OhsumedProcessor().get_train_examples("./datasets/TextClassification/ohsumed/")
    dataset['test'] = OhsumedProcessor().get_test_examples("./datasets/TextClassification/ohsumed/")
    class_labels = OhsumedProcessor().get_labels()
    scriptsbase = "TextClassification/ohsumed"
    scriptformat = "txt"
    cutoff = 0.5
    max_seq_l = 128
    batch_s = args.batch_size
...
if args.verbalizer == "kpt":
    ...
    myverbalizer = KnowledgeableVerbalizer(tokenizer, classes=class_labels).from_file(
        f"./scripts/{scriptsbase}/a_verbalizer.{scriptformat}")
